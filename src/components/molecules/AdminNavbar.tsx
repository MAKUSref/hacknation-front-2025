...
export function AdminNavbar(props: React.HTMLAttributes<HTMLElement>) {
    ...
    >
      ...
      <div className="navigation-links flex flex-wrap gap-6 max-md:hidden">
        <div className="whitespace-nowrap">
          <Link exact href={PATHS.ADMIN.DASHBOARD}>
            Strona Główna
          </Link>
        </div>
        <div className="whitespace-nowrap">
          <Link exact href={PATHS.ADMIN.ADD_PROCESSES}>
            Dodaj dokument
          </Link>
        </div>
      </div>
      <div className="shrink-0 ml-auto">
        {accessToken ? (
          <LogoutButton onClick={() => navigate(PATHS.HOME)} />
        ) : (
          <LoginButton />
        )}
      </div>
