...
export function Navbar(props: React.HTMLAttributes<HTMLElement>) {
  ...
  return (
    <nav className="p-2 w-full flex items-center" {...props}>
        <button onClick={accessToken ? handleLogout : handleLogin} className='p-3 border-black border-2 ml-auto rounded-4xl flex flex-row items-center gap-1 hover:cursor-pointer'>
            <span><img width={35} src={emblemIcon} alt="Emblem icon" /></span>
